...
const Chat = ({ chatService, kakaoService }) => {
  ...
  useEffect(() => {
    ...
    const a = geolocation.getCurrentPosition(handleSuccess);
    console.log(a);
    geolocation.watchPosition(handleSuccess, handleError, geolocationOptions);
  }, []);
  ...
  //실시간 위치추적
  useEffect(() => {
    console.log('움직임', cancelLocationWatch);
    alert('1');
    if (location && mainMap) {
      const markerPosition = kakaoService.getLatLng(
        location.latitude,
