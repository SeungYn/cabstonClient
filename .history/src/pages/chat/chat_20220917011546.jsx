...
const Chat = ({ chatService, kakaoService }) => {
  ...
  const movePosition = (posSocketData) => {
    ...
    if (posSocketData.nickname !== myName) {
      if (usersMarkers[posSocketData.nickname]) {
        usersMarkers[posSocketData.nickname].setMap(null);
      }

      const markerPosition = kakaoService.getLatLng(
        posSocketData.location.latitude,
        posSocketData.location.longitude
      );
      const marker = kakaoService.getMapMarker(markerPosition, mainMap);
      setUsersMarkers({ ...usersMarkers });
    }
  };
