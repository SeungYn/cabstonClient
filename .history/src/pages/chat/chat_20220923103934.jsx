...
const Chat = ({ chatService, kakaoService }) => {
  ...
  const onDragHeandler = (e) => {
    const move = posX - shiftX - mapParent.current.getBoundingClientRect().left
    if (move +  mapParent.current.getBoundingClientRect().width> mapParent.current.getBoundingClientRect().width) {
      move = 
    }

    mapContainerRef.current.style.left = `${
      posX - shiftX - mapParent.current.getBoundingClientRect().left
    }px`;
    console.log(
      posX,
      shiftX,
      mapParent.current.getBoundingClientRect().left,
      mapContainerRef.current.getBoundingClientRect().left,
      posX - shiftX - mapParent.current.getBoundingClientRect().left
    );
    posX = e.pageX;
  };
