...
const Chat = ({ chatService, kakaoService }) => {
    ...
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        ...
        if (entries[0].isIntersecting && hasMore) {
          console.log('로딩');
          chatService.getChatScrollVersion(1, pageId + 1).then((data) => {
            const { chats, hasMore } = data;
            setHasMore(hasMore);
            setChats((c) => [...chats, ...c]);
            setPageId((p) => p + 1);
          });
  ...
  useEffect(() => {
    ...
    chatService.getChatScrollVersion(partyid, 1).then((data) => {
      if (!flag) {
        return;
      }
      const { chats, hasMore } = data;
          ...
          <ul className={styles.chats} ref={contentRef}>
            {chats.map((item, i) => {
              return (
                <li
                  key={item.id}
                  ref={i === 0 ? firstChatElementRef : null}
                  className={
